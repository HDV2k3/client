...
//             <MainPromotions
//               data={promotionalData.data.data}
//               page={page}
//               size={size}
//             />
//           ) : (
...
import MainRoomList from "./component/MainRoomList";
import MainPromotions from "./component/MainRoomListPromotion";
import { fetchFeaturedRooms, fetchPromotionalRooms } from "./api/homeApi";
...
async function getData() {
  ...
  try {
    const [featuredData, promotionalData] = await Promise.all([
      fetchFeaturedRooms(page, size),
      fetchPromotionalRooms(page, size)
    ]);
    ...
    return {
      ...
      promotionalData: { data: { data: [] } },
    };
  }
}

export default async function HomePage() {
  const { featuredData, promotionalData } = await getData();
  const page = 1;
  const size = 8;
    ...
    <>
      <div className="mb-6 sm:mb-8">
        ...
        {featuredData?.data?.data?.length > 0 ? (
          ...
          <div>Không có phòng nổi bật</div>
        )}
      </div>
