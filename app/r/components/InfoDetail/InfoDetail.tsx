...
// Subcomponent for displaying utility items
const UtilityItem: React.FC<{ label: string }> = ({ label }) => (
  <div className="flex items-center text-gray-500 mr-2 mb-2 font-semibold text-sm">
    <FaCheckCircle className="mr-2 text-green-600" />
    {label}
  </div>
);
...
const ServicePrice: React.FC<{
  ...
  <div className="flex items-center mr-4 mb-2 whitespace-normal text-sm">
    {icon}
    <span className="mr-1 text-gray-600">{label}:</span>
    <span>{price}</span>
  </div>
...
const InfoDetail: React.FC<IProps> = ({ room }) => {
  ...
  return (
    <div className="w-full px-2 md:px-4 lg:float-right lg:w-2/3">
      <style>{glitterStyle}</style>

      {/* Title */}
      <div className="text-base md:text-lg font-bold mb-4">
        <div className="flex">
          <span className="glitter-text">{room.title}</span>
        </div>
      </div>
      ...
      {/* Room Details Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 mb-4 text-sm">
        <div className="flex items-center">
          <EnvironmentFilled className="mr-2" />
          <span>{findNameByType(room.roomInfo.district)}</span>
        </div>
      ...
      <div className="text-base md:text-xl font-bold mb-4">
        ...
        <div className="flex flex-wrap items-center">
          {hasPromotionalPrice && room.fixPrice ? (
            <>
              <span className="mr-2 text-sm md:text-xl text-red-600 line-through">
                {convertToVND(room.pricingDetails.basePrice)}
              </span>
              <span className="mr-2 text-sm md:text-xl">chỉ còn</span>
              <span className="mr-2 text-sm md:text-xl text-red-600">
                {convertToVND(room.fixPrice)}
              </span>
              <span className="text-lg md:text-xl">/ tháng</span>
            </>
          ) : (
            <>
              <span className="mr-2 text-sm md:text-xl text-red-600">
              ...
              <span className="text-sm md:text-xl">/ tháng</span>
            </>
          )}
        </div>
      </div>
      ...
      <div>
        <a
          href="https://zalo.me/0329615309"
          target="_blank"
          className="inline-flex items-center justify-center px-4 py-2 border border-black text-base font-medium rounded-md text-black bg-white hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black w-full sm:w-auto"
        >
          <Image
            src="/assets/images/icons/7044033_zalo_icon.png"
            alt="Icon ZL"
