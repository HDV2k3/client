...
export default function LayoutProfile({ children }: Props) {
  ...
  const [avatarMain, setAvatarMain] = useState<string | null>(null);
  ...
  const fetchUserData = async () => {
    try {
      if (typeof window !== "undefined") {
        ...
        const avatarMain = response.data.data.avatar;
        setAvatarMain(avatarMain);
        setFullName(fullName);
        localStorage.setItem("fullName", fullName);
        localStorage.setItem("avatarMain", avatarMain || "");
      }
    } catch (error) {
        ...
        >
          <Menu.Item
            ...
            icon={
              <UserOutlined
                style={{
                  color: path === "/profile/info" ? "black" : "#ffffff",
                }}
              />
            }
          ...
          >
            <Link
              href={"/profile/info"}
              style={{ color: path === "/profile/info" ? "black" : "#ffffff" }}
            >
              Thông Tin Cá Nhân
            </Link>
          </Menu.Item>
          <Menu.Item
            ...
            icon={
              <WalletOutlined
                style={{
                  color: path === "/profile/balance" ? "black" : "#ffffff",
                }}
              />
            }
            style={{ borderRadius: "8px", marginBottom: "8px" }}
          ...
          <Menu.Item
            ...
            icon={
              <HistoryOutlined
                style={{
                  color: path === "/profile/history" ? "black" : "#ffffff",
                }}
              />
            }
